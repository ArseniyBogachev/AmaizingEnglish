...
export const authUser = {
    ...
    actions: {
        async verify_fn(ctx){
            try {
                if (localStorage.getItem("access")){
                    await axios.post(`${process.env.VUE_APP_URL}/api/token/verify/`, {token: `${localStorage.getItem("access")}`})
        ...
        async user_data(ctx){
            try{
                const response = await axios.get(`${process.env.VUE_APP_URL}/api/v1/user/`, {headers: {"Authorization" : `Bearer ${localStorage.getItem('access')}`}})
        ...
        async user_courses_update(ctx, data){
            try{
                if (data.programm !== 'Default' && data.price !== 'Default'){
                    const response = await axios.patch(`${process.env.VUE_APP_URL}/api/v1/usercoursesupdate/`,
